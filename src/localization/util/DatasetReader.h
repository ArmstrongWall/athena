...
int getSeteroImage(ImageAndExposure* leftImg, ImageAndExposure* rightImg)
{
//    camera->Grab();//get img
//    camera->getRectImage(left, right);//RectImage
//
...
//
//
//    if(left.rows*left.cols==0 || right.rows*right.cols==0)
//    {
//        printf("cv::imread could not read  image  this may segfault. \n");
//        return false;
//    }
//
//
//    if(left.type() != CV_8U || right.type() != CV_8U)
//    {
//        printf("cv::imread CV_8U did something strange! this may segfault. \n");
//        return false;
//    }
//
//
//
//
//    MinimalImageB limg(left.cols, left.rows,(unsigned char*)left.data);
//    MinimalImageB rimg(right.cols, right.rows,(unsigned char*)right.data);
//
//
//    printf("left.rows = %d  left.cols = %d. \n",left.rows,left.cols);
//
//    memcpy(limg->data, right.data, right.rows*right.cols);
//
//    leftImg = undistort->undistort<unsigned char>(&limg,1,0, 1.0f);
//    rightImg = undistort->undistort<unsigned char>(&rimg,1,0, 1.0f);
//
//    delete limg;
//    delete rimg;
